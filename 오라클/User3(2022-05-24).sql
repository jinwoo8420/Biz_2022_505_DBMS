...
-- 학생 Entitiy 설계를 기준으로 tbl_sudent table 생성
...
/*
Relation을 table로 설계 할 때 가장 유의해야 할 부분
Entity table의 칼럼과 DOMAIN(type과 길이)을 반드시 일치시켜야 한다
*/

/* 
학생의 학과 소속 정보 Relation Table로 생성

복수의 칼럼이 UNIQUE, NOT NULL로 선언 되었다
이러한 경우 복수의 칼럼을 묶어 PK로 선언하기도 한다
이 Table에 INSERT를 수행 할 때는 데이터 검증이 잘되어
잘못된 데이터가 추가되는 것을 방지 할 수 있다

하지만 UPDATE, DELETE를 수행 할 때는
복수의 칼럼으로 조건문을 수행해야 한다
이떄 칼럼을 누락시키거나 잘못된 값으로 조건문을 수행하여
데이터 무결성이 파괴 될 수 있다

이럴때는 실제 데이터와 관련없는 단일 칼럼을 추가하고
PK를 설정하는 설계를 할 필요가 있다
*/
...
/* 
전체 칼럼에 데이터를 포함한 INSERT 할 때는 칼럼 리스트를 생략 할 수 있다
하지만 정확히 칼럼에 데이터들이 일치하는 보장이 없다
*/
INSERT INTO tbl_dept
VALUES('D0002', '전자공학', '박철수', '506');

-- 학번 1번 학생의 소속학과(D0001)를 등록
