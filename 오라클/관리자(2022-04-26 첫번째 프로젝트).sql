...
/*
1. TableSpace 생성
    오라클은 물리적 저장 공간을 TableSpace라는 이름으로 관리한다
...
생성된 사용자 권한으로 데이터가 추가된 데이터가 없어야 한다
*/
...
DBMS에 접속 할 수 있는 권한마저 없다
생성된 사용자에게는 적절한 수준의 권한을 부여해야한다
GRANT 부여
REVOKE 뺏기
*/
...
/*
DBMS에서 사용하는 SQL(Structed Query Lang) 명령
...
ALTER 명령 => 객체의 상태, 구조 등을 변경하는 명령

## DCL(Data Controll Lang) : 데이터 제어어

GRANT 명령 => 사용자에게 권한 부여 명령

REVOKE 명령 => 사용자에게서 권한 회수 명령

COMMIT 명령 ROLLBACK 명령

## DML(Data Manipulation Lang, Data Management Lang, 데이터 조작어, 관리어) : 생성된 Table에 대한 데이터 조작

C(Create) => Table 데이터 추가 : INSERT
R(Read) => Table에 저장된 데이터를 가져오기, 읽어오기 : SELECT
U(Update) => Table에 저장된 데이터의 내용을 변경하기 : UPDATE
D(Delete) => Table에 저장된 데이터를 삭제하기 : DELETE
*/
...
손상된 data가 저장된 데이터베이스 시스템은 '무결성 파괴'라고 표현한다

학습의 편의성을 위하여 사용자에게 최고 권한을 부여 할 것이다

DBA 권한은 SYSDBA보다 약간의 제약이 있는 권한이다
오라클 DBMS의 중요한 정보 접근을 제외하고 모든 것을 다 할 수 있다
다른 사용자의 tablespace에도 접근 할 수 있다
*/
